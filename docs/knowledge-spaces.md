...
如果目标会话是私聊，可以直接发送自然语言问题，效果等同于 `/ask <问题>`。如果目标会话是群聊，Bot 只响应明确命令、`@BotName 问题`，以及对 Bot 消息的回复，普通群消息不会触发查询。

多个群需要分别使用 Bot 时，在每个群组配置里填写对应的 `Bot Chat ID` 并开启“允许 Bot 查询”。Bot 查询回复会跟随该群的摘要输出语言配置；摘要推送也会优先发送到该群的 `Bot Chat ID`。如需限制谁能在该群查询，可在群组配置里填写允许查询用户白名单，支持 `@username` 或 Telegram 数字用户 ID。

查询结果会显示事实 ID。`/expire` 会把事实标记为过期，`/forget` 会把事实标记为忽略，`/restore` 可恢复过期或忽略的事实。

`/ask` 会调用已配置的 OpenAI 兼容模型，把自然语言问题解析成关键词和事实类型，例如 `/ask 谁了解炒币` 会优先查询 `skill` 类型中和“炒币”相关的事实。查询命中后，Bot 会再基于匹配事实生成简洁回答，并引用事实 ID；没有足够证据时会明确说明，不会编造答案。
...
网页端事实列表可以查看来源消息，用于确认某条事实是从哪些原始群消息抽取出来的。
...
网页端知识空间页也提供自然语言查询和自然语言维护入口。维护说明会先展示匹配事实，确认执行后才会更新状态并写入维护记录。

如果 AI 没有抽取到某条长期有价值的信息，可以在网页端人工新增事实。人工新增的事实会直接进入 active 状态，参与后续查询和摘要附加。事实列表也支持人工修正标题、类型、JSON 数据、用户、置信度、过期时间和来源消息 ID。
