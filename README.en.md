...
To use the Bot in multiple groups, set a separate `Bot Chat ID` for each group in the group settings and enable Bot queries there. Summary delivery uses the group's `Bot Chat ID` first, then falls back to the default target chat in system settings.
...
- The LLM Wiki Markdown workspace defaults to `/var/lib/tgtldr/wiki` in the app volume and can be overridden with `TGTLDR_LLM_WIKI_DIR`.
